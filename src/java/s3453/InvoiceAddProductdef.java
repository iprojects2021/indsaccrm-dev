 ...
 * @author Sachin
 */
@WebServlet(name = "InvoiceAddProductdef", urlPatterns = {"/InvoiceAddProductdef"})
public class InvoiceAddProductdef extends HttpServlet {
...
sstate=bstate;
       ...
       String invoicedueamount=Poul.escapeHtml(request.getParameter("invoicedueamount"));
...
//updating into stock
    ...
    rs.close();
      ...
      con.close();   } catch(Exception e){ e.printStackTrace(); }
         try{ if(data==null||data==""){data="0";}
             int da=Integer.parseInt(data);
             int qnttty=Integer.parseInt(qty);
             da=da-qnttty;
...
Statement st=c.createStatement();
           ResultSet rs3;
               rs3 = st.executeQuery("select  *  FROM invoiceproductlog  ");
    while(rs3.next())
          ...
          {data=rs3.getString(1);
           ...
           ++ii; 
            // st.addBatch("insert into stocklog values('"+ii+"','"+sqlTime+"','','"+owner+"','"+description[1]+"','"+description[2]+"','"+description[4]+"','Active','','"+description[3]+"','"+qty+"','','','','','','','','','','','','','Add','"+incid+"','"+newcontactdbinsert+"','"+description[0]+"','','','','','','')");
              st.addBatch("insert into invoiceproductlog values('"+ii+"','"+sqlTime+"','','"+owner+"','"+pname+"','"+pcode+"','','Active','','"+rate+"','"+qty+"','','','','','','','','','','','','','remove','"+incid+"','','"+description[0]+"','','','','"+usercid_adminid+"','"+usercid_id+"','"+contact+"')");
...
Statement st=c.createStatement();
                     ...
                     defa4=rsmd.getColumnName(47);
                      ...
                      String cal1="null",cal2="null",cal3="null",cal4="null",cal5="null",cal6="null",cal7="null",cal8="null";
                       ...
                       String clmname[]=new String[256];
                          String clmval[]=new String[256];
                       clmval[0]=sn;
                       clmval[1]=description[1];
                       clmval[2]=hsncode;
                       clmval[3]=qty;
                       clmval[4]=rate;
                       clmval[5]=unit;                    
                       clmval[6]=iamount.toString();
  ...
  st.addBatch("update invoice set  "+cal8+"='"+newcontactdbinsert+"',"+ac+"='1',"+ae+"='"+invoicedate+"',"+af+"='"+reversecharge+"',"+ag+"='"+state1+"',"+ah+"='"+statecode1+"',"+ai+"='"+transporter+"',"+aj+"='"+vehicleno+"',"+ak+"='"+supplydate+"',"+all+"='"+supplyplace+"',"+am+"='"+bname+"',"+an+"='"+baddress1+"',"+ao+"='"+baddress2+"',"+app+"='"+baddress3+"',"+aq+"='"+bcontact1+"',"+ar+"='"+bcontact2+"',"+at+"='"+bdistt+"',"+au+"='"+bstate+"',"+aw+"='"+bgstin+"',"+ay+"='"+sname+"',"+az+"='"+saddress1+"',"+ba+"='"+saddress2+"',"
          +bb+"='"+saddress3+"',"+bc+"='"+scontact1+"',"+bd+"='"+scontact2+"',"+bf+"='"+sdistt+"',"+bg+"='"+sstate+"',"+bi+"='"+sgstin+"',"
          + ""+defa1+"='"+pf+"',"+defa2+"='"+cgst+"',"+defa3+"='"+sgst+"',"+defa4+"='"+igst+"',"+defa6+"='"+discountname+"',"+defa5+"='"+discount+"',"+cal1+"='"+iproducttotal +"',"+cal2+"='"+ipf +"',"+cal3+"='"+icgst +"',"+cal4+"='"+isgst +"',"+cal5+"='"+iigst +"',"+cal6+"='"+itotalamount +"',"+cal7+"='"+inetamount +"',be_='"+amtinwords +"',bf_='"+amtinwordspaisa +"'   where id='"+incid+"'");
            for( int k=0;k<=6;k++)
         {
    st.addBatch(  "update invoice set    "+clmname[isn+k]+"='"+clmval[k]+"' where id='"+incid+"'" ); 
       
         } 
            //updating due amount
            if(invoicedueamount==null||invoicedueamount==""){invoicedueamount="0";}
            BigDecimal oldnetamount= new BigDecimal( netamount );
            BigDecimal dueamount= new BigDecimal( invoicedueamount );
            oldnetamount=inetamount.subtract(oldnetamount);
           dueamount=dueamount.add(oldnetamount);
          st.addBatch(  "update invoice set    bk_='"+dueamount+"' where id='"+incid+"'" ); 
   
       st.executeBatch(); 
       invoiceid=incid;
             msg="success";    
       if("success".equals(msg)){
      response.sendRedirect("invoicef10.jsp?2a2e2a696e636964="+Nd.Encrypt(incid)+"&msg=added");
            }
   rs3.close();
      st.close();
      c.close();     }
