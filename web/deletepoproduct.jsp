...
if (session != null) {
         ...
         Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+Poul.getPublicIP()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+" --Entering deletepoproduct.jsp---------");  
    ...
    imax=71+(imax*7);
     ...
     + "LINE=93 + \n select * from purchaseorder where id="+incid+"  ";
...
Statement st=c.createStatement();
           ResultSet rs3;
               rs3 = st.executeQuery("select  *  FROM purchaseorder where id='"+incid+"'  ");
  

         
        
         
  st.addBatch("update purchaseorder set   ak_='"+itotalproamt +"',ap_='"+ipf +"',ar_='"+icgst +"',at_='"+isgst +"',av_='"+iigst +"',ay_='"+itotalamount +"',bb_='"+inetamount +"',be_='"+amtinwords +"',bf_='"+amtinwordspaisa +"'  where id='"+incid+"'");
...
Statement st=con.createStatement();
  ...
  st.addBatch("update purchaseorder set   "+clmname[isn]+"='"+jkl+"',"+clmname[isn+1]+"='"+a1[isn+8]+"',"+clmname[isn+2]+"='"+a1[isn+9]+"',"+clmname[isn+3]+"='"+a1[isn+10]+"',"+clmname[isn+4]+"='"+a1[isn+11]+"',"+clmname[isn+5]+"='"+a1[isn+12]+"',"+clmname[isn+6]+"='"+a1[isn+13]+"'  where id='"+incid+"'");
...
Statement st=c.createStatement();
           ResultSet rs3;
               rs3 = st.executeQuery("select  *  FROM poproductlog  ");
    while(rs3.next())
        ...
        java.sql.Timestamp sqlTime=new java.sql.Timestamp(ddate.getTime());
            ...
            // st.addBatch("insert into product values('"+ii+"','"+sqlTime+"','','"+owner+"','"+description[1]+"','"+description[2]+"','"+description[4]+"','Active','','"+description[3]+"','"+qty+"','','','','','','','','','','','','','Add','"+incid+"','"+newcontactdbinsert+"','"+description[0]+"','','','','','','')");
              st.addBatch("insert into poproductlog values('"+ii+"','"+sqlTime+"','','','"+pro+"','"+cod+"','','Active','','"+rate+"','"+qt+"','','','"+amt+"','','','','','','','','','','trash','"+incid+"','','','','','','"+usercid_adminid+"','"+usercid_id+"','')");
     ...
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      ...
      response.sendRedirect("purchaseorder.jsp?incid="+incid+"&sn="+imaxxx+"#sn");
            }else{
                Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+Poul.getPublicIP()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+" -------deletepoproduct.jsp--------Redirecting To error.jsp---------");
