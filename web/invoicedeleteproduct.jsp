...
<html>
    ...
    <body>
        ...
        <%       
                 ...
                 Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+request.getRemoteAddr()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+"   --  Entring invoicedeleteproduct.jsp---");
     ...
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
                 ...
                 pf=a1[41]; vpf=a1[42];          
                 BigDecimal ipf=new BigDecimal( pf );
                 BigDecimal ivpf=new BigDecimal( vpf );
                 cgst=a1[43]; vcgst=a1[44];
                 BigDecimal icgst=new BigDecimal( cgst );
                 BigDecimal ivcgst=new BigDecimal( vcgst );
                 sgst=a1[45]; vsgst=a1[46];  
                 BigDecimal isgst=new BigDecimal( sgst );
                 BigDecimal ivsgst=new BigDecimal( vsgst );
                 igst=a1[47]; 
                 vigst=a1[48];
                 BigDecimal iigst=new BigDecimal( igst );
                 BigDecimal ivigst=new BigDecimal( vigst );
                 totalamount=a1[51]; 
                 ...
                 try
                 {
                     ...
                     st.addBatch("update invoice set   ak_='"+itotalproamt +"',ap_='"+ipf +"',ar_='"+icgst +"',at_='"+isgst +"',av_='"+iigst +"',ay_='"+itotalamount +"',bb_='"+inetamount +"',be_='"+amtinwords +"',bf_='"+amtinwordspaisa +"'  where id='"+incid+"'");
                     st.executeBatch(); 
                     st.close();
                     c.close();   
                 }
                 ...
                 {
                    String errormsg=java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"\n invoicedeleteproduct.jsp-----\n"
     + "LINE=137 + \n update invoice set   ak_='"+itotalproamt +"',ap_='"+ipf +"',ar_='"+icgst +"',at_='"+isgst +"',av_='"+iigst +"',ay_='"+itotalamount +"',bb_='"+inetamount +"',be_='"+amtinwords +"',bf_='"+amtinwordspaisa +"'  where id='"+incid+"";
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
                 } 
                 try{ 
      ...
      EmergencyEmail.send(e,errormsg);
                ...
                for(j=1;j<=nloop;j++){
                    try{
                         ...
                         if(nloop==1)
                         {jkl=0; }
                         st.addBatch("update invoice set   "+clmname[isn]+"='"+jkl+"',"+clmname[isn+1]+"='"+a1[isn+8]+"',"+clmname[isn+2]+"='"+a1[isn+9]+"',"+clmname[isn+3]+"='"+a1[isn+10]+"',"+clmname[isn+4]+"='"+a1[isn+11]+"',"+clmname[isn+5]+"='"+a1[isn+12]+"',"+clmname[isn+6]+"='"+a1[isn+13]+"'  where id='"+incid+"'");
                         st.executeBatch(); 
                         msg="success";    
                         rs3.close();
                         st.close();
     ...
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
                ...
                try
                {
                    ...
                    Statement st=c.createStatement();
                    ResultSet rs3;
                    rs3 = st.executeQuery("select  *  FROM product  ");
                    while(rs3.next())
                    {
                    ...
                    String totime=String.valueOf(sqlTime);
                    System.out.println("incid="+incid+",pro="+pro+",cod="+cod+",qty="+qt+",rate="+rate+",amt="+amt);
                    ...
                    st.addBatch("insert into product values('"+ii+"','"+sqlTime+"','','','"+pro+"','"+cod+"','','Active','','"+rate+"','"+qt+"','','','"+amt+"','','','','','','','','','','trash','"+incid+"','','','','','','"+usercid_adminid+"','"+usercid_id+"','')");
                    st.addBatch("update invoiceproductlog set type='delete',userciddelete='"+usercid_id+"',datetimedelete='"+totime+"' where invoiceid='"+incid+"' and productname='"+pro+"'  and quantity='"+qt+"'  and sellingprice='"+rate+"' and useradminid='"+usercid_adminid+"' ");
                    st.executeBatch();
                    rs3.close();
                    st.close();
                    c.close();       
     ...
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
                ...
                if("success".equals(msg)){
                    response.sendRedirect("invoice"+invoiceformat+".jsp?2a2e2a696e636964="+Nd.Encrypt(incid)+"&sn="+imaxxx+"#sn");
                }
                else{
