...
<head> <link rel="shortcut icon" href="dist/img/u.png" type="image/x-icon" /> 
  ...
  <title>New Customer | INDSAC CRM</title>
...
Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+request.getRemoteAddr()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+" --Entering newcontact.jsp----------New Customer---");
%>
    <%    String pagetype=Poul.escapeHtml(request.getParameter("pagetype"));
      if("newcontactsubmit".equals(pagetype)){
...
Statement st=c.createStatement();
    ...
    while(rs3.next())
          ...
          {SR=rs3.getString(1);
           ...
           ++i;  String requestcid=Integer.toString(i);
             st.addBatch("insert into contact values('"+i+"','"+sqlTime+"','0','"+ownr+"','"+cn+"','"+cfn+"','"+cln+"','"+department+"','"+designation+"','"+dob+"','','','"+status+"','"+rt+"','"+source+"','','"+vn+"','"+website+"','"+bwtc+"','"+mobile+"','"+alternetmobile+"','"+phone+"','"+email+"','"+oe+"','"+fax+"','"+oai+"','"+oai1+"','"+oai2+"','"+oai3+"','"+address1+"','"+address2+"','"+address3+"','"+city+"','"+pincode+"','"+state+"','"+country+"','"+notes+"','"+gstin+"','"+usercid_adminid+"','"+usercid_id+"','customer','"+sqlTime+"','','','','','','','','')");
    ...
    while(rs3.next())
          ...
          {SR=rs3.getString(1);
           ...
           ++i; 
            st.addBatch("insert into leadstatuslog values('"+i+"','"+sqlTime+"','0','','','New','"+status+"','','"+usercid_adminid+"','"+usercid_id+"','"+requestcid+"','','','','','','','','','','','','','','','','','','','','','','')");
      ...
      c.close();  }
        catch(Exception e)
        {
                  ...
                  + "LINE=145 \n insert into contact values where useradminid="+usercid_adminid+" AND customertype=New ";
       ...
       EmergencyEmail.send(e,errormsg); 
        ...
        response.sendRedirect("contactdetails.jsp");
                ...
                Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+request.getRemoteAddr()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+"   -----newcontact.jsp--------Error--------error.jsp--------");
...
<jsp:include page="dist/include/footer.jsp" />
 <% Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+Poul.getPublicIP()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+" --Leaving newcontact.jsp------");
