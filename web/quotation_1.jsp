...
<%@page import="log.Log"%>
<%@page import="blezz.QuotationInsert"%>
...
<head>
  ...
  <title> Quotation | INDSAC CRM</title>
...
Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+Poul.getPublicIP()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+" ------Entring quotation_1.jsp------");
...
Statement st=c.createStatement();
 ...
 ResultSet rs3;
            rs3 = st.executeQuery("select  count(id)   FROM quotation where c_='1' and useradminid='"+usercid_adminid+"' ");
    while(rs3.next())
        
          {businessinvoiceno=rs3.getString(1); }
               rs3 = st.executeQuery("select  id   FROM quotation ORDER BY id DESC LIMIT 1 ");
    while(rs3.next())
        
          {SR=rs3.getString(1); }
      rs3 = st.executeQuery("select c_  FROM quotation where c_='0' and useradminid='"+usercid_adminid+"' ");
    while(rs3.next())
    ...
    cdata[6]=cdata[6].concat(cdata[7]);
           ...
           if("null".equals(che1)){
         String Checkinvoic="";
        Checkinvoic=QuotationInsert.setQuotationInsert(usercid_adminid, usercid_id, contact, owner);   }
   ...
   rs3.close();
        ...
        {
           String errormsg=java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"\n quotation_1.jsp-----\n"
     + "LINE=174 \n select c_  FROM quotation where c_='0' and useradminid='"+usercid_adminid+"'  ";
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
        }
...
<div class="wrapper">
    <%
       ...
       try{
        ...
        PreparedStatement smt=con.prepareStatement("select * from quotation where id=?");
     ...
     rs.close();
           ...
           con.close();  }
                ...
                String errormsg=java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"\n quotation_1.jsp-----\n"
     + "LINE=214 \n select * from quotation where id='"+invoiceid+"'  ";
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
                }
  ...
  <div class="content-wrapper">
	...
	<div class="col-sm-3">
            <a href="invoice/purchaseorderprint.jsp?incid=<%=invoiceid %>"> <button type="button" class="btn btn-block btn-info"><i class="fa fa-fw fa-print"></i>Print</button></a>
	</div>
	...
	<div class="col-sm-3">
        ...
        <br><br>
        <div class="col-sm-3">
        <a href="contactedit.jsp?cid=">    <button  class="btn btn-block btn-info"><i class="fa fa-fw fa-check-square-o"></i>Save</button></a>
	</div>
       
         <form method="get" action="QuotationAddProduct">
...
<a href="deletequotationproduct.jsp?incid=<%=invoiceid %>&type=trsh&sn=<%=a1[jj]  %>&max=<%=iiil  %>&pro=<%=a1[jj+1]  %>&cod=<%=a1[jj+2]  %>&qt=<%=a1[jj+3]  %>&rate=<%=a1[jj+4]  %>&amt<%=a1[jj+6]  %>=#sn" > <i class="fa fa-trash-o"></i> </a>
</td>               
...
<% Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+Poul.getPublicIP()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+" --Leaving quotation_1.jsp------");
