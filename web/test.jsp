...
Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+request.getRemoteAddr()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+"   --  Entring test.jsp---");
...
Statement st=c.createStatement();
  ...
  st.addBatch("update invoice set   ak_='"+itotalproamt +"',ap_='"+ipf +"',ar_='"+icgst +"',at_='"+isgst +"',av_='"+iigst +"',ay_='"+itotalamount +"',bb_='"+inetamount +"',be_='"+amtinwords +"',bf_='"+amtinwordspaisa +"'  where id='"+incid+"'");
       ...
       st.executeBatch(); 
        ...
        {
           String errormsg=java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"\n test.jsp-----\n"
     + "LINE=148 \n update invoice set  where id='"+incid+"'  ";
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
        } 
      ...
      con.close();
          ...
          String errormsg=java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"\n test.jsp-----\n"
     + "LINE=169 \n select * from invoice set  where id='"+incid+"'  ";
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
      }
...
Statement st=c.createStatement();
    ...
    while(rs3.next())
      ...
      rs3.close();
        ...
        {
           String errormsg=java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"\n test.jsp-----\n"
     + "LINE=252 \n insert into product values '"+usercid_adminid+"','"+usercid_id+"'  ";
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
        }
      ...
      response.sendRedirect("invoice.jsp?incid="+incid+"&sn="+imaxxx+"#sn");
            }else{
                Log.writeLog(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+" ,"+request.getRemoteAddr()+" ,useradminid="+usercid_adminid+" ,usercid="+usercid_id+"   --  Entring test.jsp----------Redirecting To------------lead.jsp-----------------");
 ...
 ResultSet rs3;
    ...
    while(rs3.next())
          ...
          check=rs3.getString(3);
           ...
           ++i;  
    st.addBatch("INSERT INTO paymentinfo (id,datetime,owner,contact,invoiceid,depositorname,amountrecieved,modeofpayment,reference,balance,businessinvoiceid) VALUES ('"+i+"','"+sqlTime+"','"+owner+"','"+contact+"','"+incid+"','"+depositorname+"','"+amtreceived+"','"+mop+"','"+payref+"','"+blance+"','');"); 
    ...
    st.executeBatch(); 
       ...
       rs3.close();
        ...
        {
            String errormsg=java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"\n test.jsp-----\n"
     + "LINE=332 \n INSERT INTO paymentinfo (id,datetime,owner,contact,invoiceid,depositorname,amountrecieved,modeofpayment,reference,balance,businessinvoiceid) VALUES '"+owner+"','"+contact+"','"+incid+"','"+depositorname+"'  ";
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
...
st.addBatch("update invoice set   bk_='"+blance+"'   where id='"+incid+"'");
     ...
     Log.writeLogWarn(java.time.LocalDate.now()+" "+java.time.LocalTime.now()+"  /n"+errormsg+" /n"+e);
      EmergencyEmail.send(e,errormsg);
                ...
                if("success".equals(msg)){
      response.sendRedirect("invoice.jsp?incid="+incid+"#sn");
            }else{
